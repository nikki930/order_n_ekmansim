...
This is the version of the Ekman solver which includes effects of buoyancy in the problem equations
...
b_z_arr = np.zeros((N + 1, nx, nz))
b_x_arr = np.zeros((N + 1, nx, nz))
...
class Solver_n:
    ...
    def Jn(self, n_solve):
        ...
        #     return Jac,Jac_psi_v
            ...
            for j in range(0, n+1):
                ...
                J3 += psi_x_arr[j, :] * b_z_arr[n - j, :] - psi_z_arr[j, :] * b_x_arr[n - j, :]
            ...
            Jac_temp3 = domain.new_field()
            gslices = domain.dist.grid_layout.slices(scales=1)
            Jac_temp3['g'] = J3[gslices[0]]

            return Jac_temp1, Jac_temp2, Jac_temp3
    ...
    def analysis(self, FolderName):
        ...
        with h5py.File(folder_n + '/' + folder_n_sub + '_s1/' + folder_n_sub + '_s1_p0.h5',
            ...
            b_arr[self.n, :, :] = np.array(file['tasks']['<b>'])
            b_z_arr[self.n, :, :] = np.array(file['tasks']['<bz>'])
            b_x_arr[self.n, :, :] = np.array(file['tasks']['<bx>'])#
