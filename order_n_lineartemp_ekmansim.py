...
N = 20 # the order up to which the model runs
...
class Solver_n:
    ...
    def eqns(self, state_var):
        ...
        problem.add_equation("(dx(dx(v)) + dz(vz))*nu - r*(1/H)*integ(v,'z')  -f*u = Jac_psi_v")  # nu* grad^2 v  - fu=0
        problem.add_equation("(dx(dx(zeta)) + dz(zetaz))*nu + f*vz = Jac")  # nu* grad^2 zeta + fv_z=0
    ...
    def plotting(self, folder, FileName):
        ...
        if self.n == 0:
            ...
            # print(psi_arr[0,:,:])
            CS1 = plt.contourf(Z, X, psi_arr_corrected[0, :, :], cmap='seismic',vmax=0.125)
            cbar = fig.colorbar(CS1)
        else:
            psi_arr_corrected[self.n, :, :] = psi_arr_corrected[self.n - 1, :, :] + psi_arr[self.n, :, :]
            # print(psi_arr_corrected[1,:,:])

            CS2 = plt.contourf(Z, X, psi_arr_corrected[self.n, :, :], cmap='seismic',vmax=0.125)
